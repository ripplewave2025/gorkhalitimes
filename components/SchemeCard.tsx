    ...
    return (
        <article className="surface-card rounded-[2rem] p-5">
            ...
            <div className="mt-4 space-y-3 text-sm leading-6 text-brand-muted">
                <p><span className="font-medium text-brand-ink">{language === 'ne' ? '???? ????:' : 'Who it is for:'}</span> {getLocalizedText(scheme.whoItsFor, contentLanguage, fallbackLanguage)}</p>
                <p><span className="font-medium text-brand-ink">{language === 'ne' ? '??? ???:' : 'Benefit summary:'}</span> {getLocalizedText(scheme.benefitSummary, contentLanguage, fallbackLanguage)}</p>
                <p><span className="font-medium text-brand-ink">{language === 'ne' ? '???????:' : 'Eligibility:'}</span> {getLocalizedText(scheme.eligibilitySnapshot, contentLanguage, fallbackLanguage)}</p>
            </div>
            <div className="mt-4 flex flex-wrap gap-3">
                <a href={scheme.learnMoreUrl} target="_blank" rel="noreferrer" className="btn-secondary justify-center">{language === 'ne' ? '?? ??????????' : 'Learn more'}</a>
