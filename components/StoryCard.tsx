...
import { Bookmark, ChevronDown, ChevronUp, Headphones, ShieldCheck, Share2 } from 'lucide-react';
...
export default function StoryCard({ story, onNext, onPrevious }: StoryCardProps) {
    ...
    const handleShare = async () => {
        const title = getLocalizedText(story.headline, language);
        const text = getLocalizedText(story.summaryShort, language);
        ...
        <article className="surface-card overflow-hidden rounded-[2rem]">
            <div className="relative h-[22rem] w-full overflow-hidden md:h-[26rem]">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-5 text-white">
                    ...
                    <h2 className="max-w-2xl text-2xl font-semibold leading-tight md:text-3xl">{getLocalizedText(story.headline, language)}</h2>
                    <p className="mt-3 max-w-2xl text-sm leading-6 text-white/85">{getLocalizedText(story.summaryShort, language)}</p>
                </div>
            </div>
            <div className="space-y-4 p-5">
                <div className="flex flex-wrap items-center gap-2 text-xs text-brand-muted">
                    ...
                    <span>•</span>
                    <span>{story.sourceIds.length} sources visible</span>
                    <span>•</span>
                ...
                {guardianNote ? (
                    <div className="rounded-2xl bg-brand-green-soft p-4 text-sm leading-6 text-brand-ink">
                        ...
                        <p>{getLocalizedText(guardianNote.text, language)}</p>
                    </div>
                ) : null}
                <div className="grid gap-3 md:grid-cols-2">
                    <button type="button" className="btn-primary justify-center">
                        <Headphones size={16} />
                        {getLocalizedText(appCopy.actions.listen, language)}
                    </button>
