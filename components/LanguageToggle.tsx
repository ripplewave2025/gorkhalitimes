...
export default function LanguageToggle({ mode = 'compact' }: LanguageToggleProps) {
    ...
    return (
        <label className="surface-card inline-flex items-center gap-3 rounded-full px-3 py-2 text-sm text-brand-ink">
            <span className="text-xs font-semibold uppercase tracking-[0.16em] text-brand-muted">Lang</span>
            <select value={language} onChange={(event) => setLanguage(event.target.value as typeof language)} className="bg-transparent pr-2 text-sm text-brand-ink outline-none">
                {SUPPORTED_LANGUAGES.map((item) => (
